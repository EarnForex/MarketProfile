    ...
    for (int intraday_i = 0; intraday_i < IntradaySessionCount_tmp; intraday_i++)
    {
        ...
        else
        {
            ...
            if (IDStartTime[intraday_i] < IDEndTime[intraday_i])
            {
                ...
                else while ((sessionend < rates_total) && ((TimeHour(Time[sessionend]) * 60 + TimeMinute(Time[sessionend]) >= IDEndTime[intraday_i]) || ((TimeDayOfWeek(Time[sessionend]) == 6) && (SaturdaySunday == Append_Saturday_Sunday))))
                    {
                        sessionend++;
                    }
            ...
            else if (IDStartTime[intraday_i] > IDEndTime[intraday_i])
            {
                ...
                else if ((SaturdaySunday == Append_Saturday_Sunday) && (TimeDayOfWeek(Time[sessionstart]) == 0))
                {
                    ...
                    while ((sessionend < rates_total) && (TimeDayOfWeek(Time[sessionend]) == 1) && (TimeHour(Time[sessionend]) * 60 + TimeMinute(Time[sessionend]) >= IDEndTime[intraday_i]))
                    {
                        sessionend++;
                    }
                }
